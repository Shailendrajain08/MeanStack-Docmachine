    ...
    if (dataObj) {
        ...
        validators.generateJWTToken(dataObj.emailId, (err, res) => {
            if (err) {
                ...
                try {
                    const forgotemailLink = process.env.LIVE_URL + '/updatePassword/' + res.split(" ")[1];
                    ...
                    const html = EmailFormat.generalFormat({ html: content, heading: "Forgot Password", host: process.env.LIVE_URL });
